  ...
  return (
      ...
      <h1 className='text-2xl font-bold mb-6 text-yellow-800'>SIGN-UP</h1>
        <div className='flex flex-col gap-6 text-lg font-medium text-yellow-800'>
              ...
              <input type="text" className='outline-none border-yellow-800 border-b-2 focus:border-yellow-500 border-solid pb-3 max-w-[26rem] bg-transparent w-[90vw]' placeholder='enter email' id='emailID' value={email} onChange={(e) => setEmail(e.target.value)}  />
              </div>
              <div className='flex flex-col gap-2'>
           ...
           <button type='submit' className='bg-orange-500 mt-4 text-orange-800 font-medium  text-center text-md text-white p-3 rounded-lg place-self-start mb-5' onClick={()=>submitInfo({name,email,password,confirm})}>REGISTER</button>
